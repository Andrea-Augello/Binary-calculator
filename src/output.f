...
: DISPLAY_MASK ( n -- mask ) 
	...
	BEGIN
		...
		ROT AND 			\ adds the newly computed mask to the previous result
		R> DUP
		DISPLAY_SIZE <=
	UNTIL
	DROP SWAP ROT ;			
...
: CLEAR ( -- )
	[ 1 DISPLAY_SIZE LSHIFT 1 - DISPLAY_MASK ] IMMEDIATE 	\ mask with every display
																			\ bit set to 1
	GPCLR0 ! 
	OVERFLOW OFF
	NEGATIVE OFF ;
: SHOW ( status n -- )  ;
	CLEAR  DISPLAY_MASK GPSET0 ! 
